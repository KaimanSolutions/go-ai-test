...
public sealed class BrandingSettingsService
{
    ...
    public BrandingSettingsService(FormBuilderDbContext context) => _context = context;

    public void EnsureDefaultSettings()
    {
        if (_context.BrandingSettings.Any()) return;
        _context.BrandingSettings.Add(new BrandingSettings());
        _context.SaveChanges();
    }

    public BrandingSettings GetSettings() =>
        _context.BrandingSettings.OrderBy(b => b.Id).FirstOrDefault() ?? new BrandingSettings();
