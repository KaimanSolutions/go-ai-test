...
public sealed class ProfileSettingsService
{
    ...
    public ProfileSettingsService(FormBuilderDbContext context)
    {
        _context = context;
    }
    ...
    {
        if (_context.ProfileSettings.Any())
            return;
    ...
    {
        return _context.ProfileSettings.OrderBy(p => p.Id).FirstOrDefault() ?? new ProfileSettings();
    }
